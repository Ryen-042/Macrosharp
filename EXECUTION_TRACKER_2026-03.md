...
| Phase 5 - Host Architecture, Readability, And Feature Delivery | 2026-05-09 to 2026-05-22 | In Progress | 2026-03-21 | 2026-05-22 |  | 88 | P5-2 progressing: Program bootstrap/lifecycle cleanup now includes startup banner/message-loop helpers, class-level hotkey metadata constants, tray menu builder extraction, main-configuration watcher setup, text-expansion setup wiring, keyboard/Scroll Lock handler setup methods, hotkey registration setup wiring, runtime notifier/cleanup helpers, and encapsulated burst-click controller setup |
...
| P5-2 | Reduce Program to bootstrap and lifecycle orchestration |  | In Progress | 2026-05-14 | 2026-03-21 |  | P5-1 | No | None | Progressed with startup banner/message loop extraction, class-level hotkey source/throttle constants, local BuildTrayMenu() extraction, SetupMainConfigurationWatching(), SetupTextExpansion(), extracted keyboard handler setup methods (terminal logging and ESC burst-stop), SetupScrollLockMouseHandler(), SetupHotkeyRegistrations(), SetupRuntimeNotifiers(), CleanupAndExit(), and SetupBurstClickController() extraction with thin wrapper methods to reduce Main body complexity while preserving runtime behavior |
...
- Plan updates applied: Recorded corrected P3-3 policy decision, completed bounded dispatch implementation (P3-3), high-frequency policy mapping (P3-4), scheduler cadence updates for P4-1/P4-2, and completed P5-1 by moving host-shared orchestration code plus all hotkey groups into Macrosharp.Hosts.Shared.
